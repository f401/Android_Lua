...
public class CodeEditor extends View implements ContentListener, Formatter.FormatResultReceiver {
    ...
    protected void applyAttributeSets(AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        TypedArray array = getContext().obtainStyledAttributes(attrs, R.styleable.CodeEditor, defStyleAttr, defStyleRes);
    ...
    public void setEditorLanguage(@Nullable Language lang) {
        ...
        // Setup new one
        AnalyzeManager mgr = lang.getAnalyzeManager();
    ...
    protected long findLeadingAndTrailingWhitespacePos(ContentLine line) {
        char[] buffer = line.getBackingCharArray();
    ...
    public void indentLines(boolean onlyIfSelected) {
        ...
        for (int i = cursor.getLeftLine(); i <= cursor.getRightLine(); i++) {
            ...
            final int spaces = spaceCount + (tabCount * tabWidth);
            final int endColumn = spaceCount + tabCount;

            final int requiredSpaces = tabWidth - (spaces % tabWidth);
            ...
            if (requiredSpaces == 0) {
                ...
                text.insert(i, endColumn, Strings.repeat(" ", requiredSpaces));
            }
        }
        text.endBatchEdit();
    }
    ...
    public void deleteText() {
        ...
        if (cur.isSelected()) {
            ...
            if (props.deleteEmptyLineFast || (props.deleteMultiSpaces != 1 && col > 0 && text.charAt(line, col - 1) == ' ')) {
                ...
                char[] text = this.text.getLine(cur.getLeftLine()).getBackingCharArray();
    ...
    public long getPointPositionOnScreen(float x, float y) {
        y = Math.max(0, y);
        List<CodeBlock> stuckLines = renderer.lastStuckLines;
        if (stuckLines != null) {
            if (y < stuckLines.size() * getRowHeight()) {
                int index = (int) (y / getRowHeight());
    ...
    public int getLineSpacingPixels() {
        android.graphics.Paint.FontMetricsInt metrics = renderer.metricsText;
    ...
    public PointerIcon onResolvePointerIcon(MotionEvent event, int pointerIndex) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            if (event.isFromSource(InputDevice.SOURCE_MOUSE)) {
                ...
                long res = RegionResolver.resolveTouchRegion(this, event, pointerIndex);
                int region = IntPair.getFirst(res);
                boolean inbound = IntPair.getSecond(res) == RegionResolver.IN_BOUND;
    ...
    protected void onCreateContextMenu(ContextMenu menu) {
        ...
        final PointF pos = touchHandler.getLastContextClickPosition();
        if (pos == null) {
            return;
        }
        long charPos = getPointPositionOnScreen(pos.x, pos.y);
