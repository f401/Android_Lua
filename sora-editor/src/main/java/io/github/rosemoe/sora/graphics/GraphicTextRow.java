...
public class GraphicTextRow {
    ...
    public void buildMeasureCache() {
        MeasureCacheItem cacheItem = context.getCache().getOrCreateMeasureCache(line);
        ...
        // Generate prefix sum
        float[] cache = cacheItem.getWidths();
    ...
    public long findOffsetByAdvance(int start, float advance) {
        if (useCache) {
            MeasureCacheItem cacheItem = context.getCache().queryMeasureCache(line);
        ...
        // Find in each region
        long lastStyle = 0L;
        char[] chars = text.getBackingCharArray();
    ...
    public float measureText(int start, int end) {
        ...
        if (useCache) {
            MeasureCacheItem cache = context.getCache().queryMeasureCache(line);
