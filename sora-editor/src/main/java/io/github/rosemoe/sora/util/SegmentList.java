...
public class SegmentList<T> extends AbstractList<T> {
    ...
    private FindResult<T> getSegment(int index) {
        ...
        for (int i = 0, j = segments.size() - 1; i <= j; i++, j--) {
            Segment<T> block = segments.get(i);
    ...
    private FindResult<T> getSegmentMut(int index) {
        FindResult<T> res = getSegment(index);
        res.segment = ensureMutable(res.blockIndex);
        return res;
    }

    private Segment<T> ensureMutable(int segIdx) {
        Segment<T> block = segments.get(segIdx);
        Segment<T> n = block.toMutable();
        if (block != n) {
    ...
    public T set(int index, T element) {
        ...
        FindResult<T> result = getSegmentMut(index);
    ...
    public void add(int index, T element) {
        ...
        if (result.segment.size() >= segmentCapacity) {
            int divPoint = segmentCapacity / 2;
            Segment<T> seg = new Segment<T>(segmentCapacity);
    ...
    public T remove(int index) {
        checkAccessIndex(index);
        FindResult<T> result = getSegmentMut(index);
        T res = result.segment.remove(index - result.offset);
        length--;
    ...
    public T get(int index) {
        checkAccessIndex(index);
        FindResult<T> result = getSegment(index);
    ...
    protected void removeRange(int fromIndex, int toIndex) {
        ...
        FindResult<T> res = getSegment(fromIndex);
        int offset = res.offset;
        int index = res.blockIndex;
        Segment<T> seg = res.segment;
        ...
        while (toIndex - offset > 0 && index < segments.size()) {
            ...
            if (fromIndex <= offset && toIndex >= offset + segLength) {
                ...
                List<T> sub = seg.subList(Math.max(fromIndex - offset, 0), Math.min(toIndex - offset, segLength));
                sub.clear();
                index++;
            }
            offset += segLength;
    ...
    public SegmentList<T> shallowCopy() {
        SegmentList<T> list = new SegmentList<T>(segmentCapacity);
        list.segments.clear();
        for (SegmentList.Segment<?> seg : segments) {
            seg.retain();
        }
    ...
    private void mergeSegment(int seg1, int seg2) {
        ...
        Segment<T> pre = segments.get(seg1);
        Segment<T> aft = segments.get(seg2);
    ...
    private void adjustElements(int segIdx, Segment<T> mutCur) {
        if (segIdx > 0) {
            Segment<T> pre = segments.get(segIdx - 1);
            if (pre.isMutable() && pre.size() <= segmentCapacity * 4 / 5 && mutCur.size() > segmentCapacity * 4 / 5) {
                List<T> sub = mutCur.subList(0, segmentCapacity * 4 / 5 - pre.size());
                pre.addAll(sub);
                sub.clear();
            }
        }
    ...
    private static class Segment<T> extends ArrayList<T> implements ShareableData<Segment<T>> {
        ...
        public Segment<T> copy() {
            Segment<T> res = new Segment<T>(this.size());
            res.addAll(this);
            return res;
        }
    }
